...
`)
	...
	t.Run("sample hooks config", func(t *testing.T) {
		dir := t.TempDir()
		configPath := filepath.Join(dir, ".github", "teraflow.yml")
		writeCmdTestFile(t, configPath, `version: "1"
hooks:
...
`)
		...
		root.SetArgs([]string{"--config", configPath, "hook", "validate"})
		if err := root.Execute(); err != nil {
			t.Fatalf("expected sample hooks config to be valid, got: %v", err)
		}
	})
}
...
func TestHookRunDryRunDiscussionCreated(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, ".github", "teraflow.yml")
	writeCmdTestFile(t, configPath, `version: "1"
hooks:
...
`)
	...
	root.SetArgs([]string{
		"--config", configPath,
		"--format", "json",
		"hook", "run", "on_discussion_created",
		"--author", "alice",
		"--category", "Requirements",
		"--discussion-id", "D_kwDOHOOK",
	...
	if got.Event != "on_discussion_created" {
		t.Fatalf("unexpected event: %s", got.Event)
	}
	if !got.DryRun {
		t.Fatalf("expected dry run true")
